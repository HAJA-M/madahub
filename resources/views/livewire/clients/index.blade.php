...
new #[Layout('layouts.app')] class extends Component
{
    ...
    public function supprimer(int $id): void
    {
        ...
        session()->flash('succes', __('Client supprimé.'));
    }
}; ?>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-4">
                <input type="text" wire:model.live.debounce.300ms="recherche"
                       placeholder="{{ __('Rechercher un client (nom, email, téléphone)...') }}"
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-900/50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('Contact') }}</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('Ventes') }}</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse ($clients as $client)
                            <tr wire:key="client-{{ $client->id }}">
                                ...
                                <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400 capitalize">{{ __($client->type) }}</td>
                                ...
                                <td class="px-4 py-3 text-right text-sm space-x-2 whitespace-nowrap">
                                    <a href="{{ route('clients.edit', $client) }}" wire:navigate class="text-brand-blue-700 hover:underline">{{ __('Modifier') }}</a>
                                    @if ($confirmationSuppression === $client->id)
                                        <button wire:click="supprimer({{ $client->id }})" class="text-red-600 hover:underline">{{ __('Confirmer ?') }}</button>
                                        <button wire:click="$set('confirmationSuppression', null)" class="text-gray-400 hover:underline">{{ __('Annuler') }}</button>
                                    @else
                                        <button wire:click="demanderSuppression({{ $client->id }})" class="text-gray-400 hover:text-red-600">{{ __('Supprimer') }}</button>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
                                    {{ __('Aucun client pour le moment.') }}
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
